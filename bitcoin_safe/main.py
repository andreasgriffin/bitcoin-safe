...
class MainWindow(Ui_MainWindow):
    ...
    def export_wallet_for_coldcard(self, wallet=None):
    ...
    def dialog_open_tx_from_str(self):
        ...
        tx_dialog = ImportDialog(network=self.config.network_config.network, on_open=process_input)
        tx_dialog.show()
    ...
    def create_qtprotowallet(self, m_of_n: Tuple[int, int]) -> Optional[QTProtoWallet]:
        def create_wallet(qtprotowallet: QTProtoWallet):
            wallet = Wallet.from_protowallet(
                qtprotowallet.protowallet,
                qtprotowallet.wallet_id,
                self.config,
                tutorial_step=qtprotowallet.wallet_steps.step_bar.current_step + 1,
            )
        ...
        protowallet = ProtoWallet(
            threshold=m, keystores=[None for i in range(n)], network=self.config.network_config.network
        )
        ...
        qtprotowallet.signal_create_wallet.connect(lambda: create_wallet(qtprotowallet))

        # tutorial
        qtprotowallet.wallet_steps = WalletSteps(
            wallet_tabs=qtprotowallet.tabs,
            qtwalletbase=qtprotowallet,
            signal_create_wallet=qtprotowallet.signal_create_wallet,
        )
        qtprotowallet.outer_layout.insertWidget(0, qtprotowallet.wallet_steps)
    ...
    def add_qt_wallet(self, wallet: Wallet) -> QTWallet:
        ...
        assert (
            wallet.id not in self.qt_wallets
        ), f"A wallet with id {wallet.id} is already open. Please close the existing wallet."
        ...
        qt_wallet.wallet_steps = WalletSteps(
            ...
            signal_create_wallet=qt_wallet.wallet_descriptor_ui.signal_qtwallet_apply_setting_changes,
            qt_wallet=qt_wallet,
        )
    ...
    def toggle_tutorial(self):
        ...
        if qt_wallet.wallet_steps:
            show_tutorial = not qt_wallet.wallet_steps.isVisible()
            qt_wallet.wallet_steps.setVisible(show_tutorial)

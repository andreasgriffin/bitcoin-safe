...
class BlockButton(QPushButton):
    ...
    def update_title(self, title:str):
        self.labels[0].setText(f"<span style='color: black; font-size: 16px;'>{title}</span>")

    def update_median_fee(self, median_fee:float):
        self.labels[1].setText(f"<span style='color: black; font-size: 12px;'>~{int(median_fee)} sat/vB</span>")

    def update_fee_range(self, min_fee, max_fee):
        self.labels[2].setText(f"<span style='color: darkorange; font-size: 10px;'>{int(min_fee)} - {int(max_fee)} sat/vB</span>")

    def update_bottom_text(self, blocknumber):
        if blocknumber <6:
            self.labels[3].setText(f"<span style='color: black; font-size: 12px;'>~in {(blocknumber)*10} min</span>")
        else:
            self.labels[3].setText(f"<span style='color: black; font-size: 12px;'>~in {round(blocknumber/6)} hours</span>")
...
class MempoolButtons(QObject):
    ...
    def __init__(self, mempool_data:MempoolData, button_count=3, fee=1, parent=None) -> None:
        super().__init__()
        ...
        self.button_group = VerticalButtonGroup(button_count=button_count, parent=parent)
    ...
    def _fill_buttton_text(self):
        ...
        def fees_of_depths(depths):
            fee_borders_indizes = [index_of_sum_until_including(self.mempool_data.data[:,1], depth) for depth in depths]
            return [self.mempool_data.data[i,0] for i in fee_borders_indizes]
        
        depths = np.arange(len(self.button_group.buttons)+1)*1e6
        block_fee_borders = fees_of_depths(depths)     
        self.median_block_fee_borders = fees_of_depths(depths + 0.5e6)   
...
class MempoolProjectedBlock(QObject):
    signal_click = Signal(float)
    def __init__(self, mempool_data:MempoolData, fee=1, parent=None) -> None:
        super().__init__()
    ...
    def _fill_buttton_text(self):
        ...
        block_number = fee_to_blocknumber(self.mempool_data.data, self.fee)

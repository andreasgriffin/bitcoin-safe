...
# flatpak-builder encodes the absolute build-dir path into OSTree ref names.
# Inside the Docker container the checkout lives under a long path
# (/opt/wine64/drive_c/bitcoin_safe/...), which pushes the generated ref name
# past the filesystem path limit. Use a short, fixed build root so ref names
# stay short regardless of the checkout location.
BUILD_ROOT="${BITCOINSAFE_FLATPAK_BUILD_ROOT:-/tmp/bitcoin-safe-flatpak-build}"
...
SOURCE_STAGING_DIR="${BUILD_ROOT}/source-tree"
GENERATED_REPO_DIR="${BUILD_ROOT}/generated-repo"
...
METADATA_PROBE_SOURCE_DIR="${BUILD_ROOT}/metadata-probe-src"
METADATA_PROBE_BIN_DIR="${BUILD_ROOT}/metadata-probe-bin"
METADATA_PROBE_MANIFEST_PATH="${BUILD_ROOT}/metadata-probe.yml"
METADATA_PROBE_BUILDER_DIR="${BUILD_ROOT}/metadata-probe-builder"
METADATA_PROBE_REPO_DIR="${BUILD_ROOT}/metadata-probe-repo"
METADATA_PROBE_STATE_DIR="${BUILD_ROOT}/metadata-probe-state"
...
write_metadata_probe_files() {
    ...
    cp "${SOURCE_STAGING_DIR}/tools/build_linux/flathub_flatpak/${APP_ID}.svg" "${METADATA_PROBE_SOURCE_DIR}/${APP_ID}.svg"
    cp "${PROJECT_ROOT}/tools/resources/icon-128.png" "${METADATA_PROBE_SOURCE_DIR}/icon-128.png"
    cp "${PROJECT_ROOT}/tools/build_linux/flathub_flatpak/${APP_ID}.desktop" \
        "${METADATA_PROBE_SOURCE_DIR}/linux-bitcoin-safe.desktop"
    cp "${SOURCE_STAGING_DIR}/tools/build_linux/flathub_flatpak/${APP_ID}.metainfo.xml" \
...
stage_source_tree() {
    ...
    if git -C "${PROJECT_ROOT}" rev-parse --show-toplevel >/dev/null 2>&1; then
        ...
        tar -C "${PROJECT_ROOT}" \
            ...
            --exclude='__pycache__' \
            --exclude='*.pyc' \
            --exclude='tools/build_linux/flathub_flatpak/build' \
...
generate_manifest_repo() {
    local generator_module
    local generator_python

    generator_module="tools.build_linux.flathub_flatpak.populate_cli"
    ...
    (
        cd "${PROJECT_ROOT}"
        "${generator_python}" -m "${generator_module}" \

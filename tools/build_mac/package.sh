...
CONTRIB_OSX="$PROJECT_ROOT/tools/build_mac"
...
# cp -r stamps the staged tree with the current wall-clock time. genisoimage
# embeds file mtimes into the image, so two runs of the same app at different
# times would produce different DMG hashes. Normalize to the same fixed
# timestamp used by the rest of the macOS reproducible build path.
find "$staging_dir" -exec touch -h -t '200101220000' {} +

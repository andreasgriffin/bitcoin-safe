...
This guide explains how to build Bitcoin_Safe binaries for macOS systems.
...
- The [bundled version of Qt](https://github.com/andreasgriffin/bitcoin-safe/issues/3685) also
...
- If you want to build binaries that conform to the macOS "Gatekeeper", so as to
  ...
  See [#6128](https://github.com/andreasgriffin/bitcoin-safe/issues/6128).
...
- The unix username should be `vagrant`, and `bitcoin_safe` should be cloned directly
  to the user's home dir: `/Users/vagrant/bitcoin_safe`.
...
- Installing extraneous brew packages can result in build differences.
  ...
  So having a dedicated "bitcoin_safe binary builder macOS VM" is recommended.
- Make sure that you are building from a fresh clone of bitcoin_safe
...
#### 2. Build Bitcoin_Safe

    cd bitcoin_safe
    ./tools/build-mac/make_osx.sh

This creates both a folder named Bitcoin_Safe.app and the .dmg file (both unsigned).
...
also run the `sign_osx.sh` script:
    ...
    CODESIGN_CERT="Developer ID Application: Bitcoin_Safe Technologies GmbH (L6P37P7P56)" \
    APPLE_TEAM_ID="L6P37P7P56" \
    APPLE_ID_USER="me@email.com" \
    APPLE_ID_PASSWORD="1234" \
    ./tools/build-mac/sign_osx.sh
...
2. Use the provided `compare_dmg` script to compare the binary you built with
   the official release binary.
    ```
    $ ./tools/build-mac/compare_dmg dist/bitcoin_safe-*.dmg bitcoin_safe_dmg_official_release.dmg
    ```
